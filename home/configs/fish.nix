...
let
  ...
  '';
  # On WSL, fish is exec'd from bash, which adds Nix to the PATH, so only add it from fish on
  # non-NixOS home-manager-only systems.
  homeManagerOnlyInit = optionalString
    (cfg.homeManagerOnlyCompatibility && !config.veritas.profiles.wsl.enable)
    ''
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    programs.fish = {
      enable = true;
      interactiveShellInit = homeManagerOnlyInit + commonInit;
      package = pkgs.fish;
